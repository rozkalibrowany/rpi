...
#***********************************************************#
    ...
    def setItemsQuantity(self, items, added):
        if (added):
            print ("\t {}: {} {}".format(DICT_ADDED, items, DICT_ITEM))
            self.itemsLeft += items
        else:
            print ("\t {}: {} {}".format(DICT_TAKE_OFF, items, DICT_ITEM))
            self.itemsLeft -= items
...
def readDictionary():
    try:
        ...
        DICT_UPTIME = config.get("DICTIONARY", "DICT_UPTIME")
    ...
    for sensor in sensorList:
        ...
        if (sensor.analysing == True and sensor.timer.elapsedTime() > TIME_ALERT_LIMIT):
            ...
            if (abs(diff) > UNIT_WEIGHT):
                onChangeItemEmail(sensor.name, sensor.getItems(abs(diff)), abs(diff), sensor.isItemTakeOff(diff))
                sensor.setItemsQuantity(sensor.getItems(abs(diff)), sensor.isItemTakeOff(diff))
